"""Keras sliding-window molecule generation. Mirrors the PyTorch variant
in geneva2s.torch.generate.
"""
...
def predict_batch_seeds(
    ...
    """Same algorithm as the PyTorch variant — sliding-window per-token rollout.
    Returns list[str] of generated SMILES."""
    try:
        import tensorflow as tf
    except ImportError as e:
        raise ImportError("pip install tensorflow==2.15.1 tensorflow-metal==1.1.0") from e
    ...
    while len(mols) < ncollect:
        ...
        while any(active):
            ...
            if stop_outer:
                break
            x = tf.concat(
                [x[:, 1:], tf.constant(next_indices)[:, None]], axis=1
            )
